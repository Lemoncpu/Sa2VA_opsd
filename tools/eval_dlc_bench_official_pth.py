...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export Sa2VA DLC-Bench predictions in the official NVlabs/describe-anything format from a config + .pth checkpoint."
    )
...
def main():
    ...
    model = load_opsd_model_from_pth(
    ...
    with torch.no_grad():
        for index in range(start, len(annotations), step):
            ...
            exported += 1
            print(f"[{exported:04d}] ann_id={ann_id} status={description.status:<18} caption={caption!r}")

    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(predictions, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"[done] wrote official predictions to {output_path}")

    if args.debug_output:
        debug_output_path = Path(args.debug_output)
        debug_output_path.parent.mkdir(parents=True, exist_ok=True)
        debug_output_path.write_text(json.dumps(debug_items, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"[done] wrote debug sidecar to {debug_output_path}")
