...
if [[ -z "${OUTPUT_DIR}" ]]; then
  ...
  ckpt_stem="${ckpt_name%.*}"
  OUTPUT_DIR="${PROJECT_ROOT}/work_dirs/dlc_bench_eval_${ckpt_stem}"
fi
...
CMD=(
  "${PYTHON_BIN}"
  "${PROJECT_ROOT}/tools/eval_dlc_bench_official_pth.py"
  --config "${CONFIG_PATH}"
  --checkpoint "${CHECKPOINT_PATH}"
  --base-model-path "${BASE_MODEL_PATH}"
  --tokenizer-path "${TOKENIZER_PATH}"
  --data-root "${DATA_ROOT}"
  --output "${OUTPUT_DIR}/pred.json"
  --debug-output "${OUTPUT_DIR}/pred_debug.json"
  --device "${DEVICE}"
  --start "${START:-0}"
  --step "${STEP:-1}"
)
