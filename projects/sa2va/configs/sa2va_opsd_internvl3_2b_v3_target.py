...
path = getenv("SA2VA_PUBLIC_2B_MODEL_PATH", "./pretrained/Sa2VA-4B")
tokenizer_path = getenv("SA2VA_PUBLIC_2B_TOKENIZER_PATH", path)
npz_dir = getenv("SA2VA_PUBLIC_2B_NPZ_DIR", "./data/npz")
device = getenv("SA2VA_PUBLIC_2B_DEVICE", "auto")
...
train_dataset = dict(
    type=Sa2VAOpsdNPZDatasetV2,
    npz_dir=npz_dir,
    prefix="masklet_data",
    repeats=1,
    shuffle=False,
    student_question=(
        "<image>Describe the main object or person marked by region1 using a short, precise caption. "
        "Focus on the target itself: its category, distinctive appearance, color, material or texture, "
        "and other key visual traits that uniquely identify it. Avoid long scene descriptions and avoid "
        "complex relationships unless they are necessary to identify the target. Answer with one concise "
        "descriptive sentence only. Do not output placeholder tags or segmentation tokens."
    ),
)
...
work_dir = getenv("SA2VA_PUBLIC_2B_WORK_DIR", "./work_dirs/sa2va_opsd_internvl3_2b_v3_target")
