...
if errors:
    raise SystemExit(
        "Active environment is missing OPSD DLC export dependencies. "
...
echo "Exporting RefCOCO DLC choose-one routes with:"
...
if [[ "${GPUS}" -eq 1 ]]; then
  ...
  "${PYTHON_BIN}" tools/export_opsd_dlc_routes.py "${CONFIG}" "${EXPORT_ARGS[@]}"
else
  PYTHONPATH="${ROOT_DIR}:${PYTHONPATH:-}" \
  CUDA_VISIBLE_DEVICES="${CUDA_DEVICE_IDS}" \
  PORT="${PORT}" \
  ...
  bash tools/dist.sh export_opsd_dlc_routes "${CONFIG}" "${GPUS}" "${EXPORT_ARGS[@]}"
fi
